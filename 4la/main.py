class Graph:
    ...
    def remove_hanging_vertices(self): # Висячие вершины
        ...
        """
        new_adjacency_list = {v: adj.copy() for v, adj in self.adjacency_list.items()}

        # Находим все висячие вершины
        hanging_vertices = [v for v, adj in new_adjacency_list.items() if len(adj) == 1]

        # Удаляем висячие вершины
        for vertex in hanging_vertices:
            # Удаляем все упоминания висячей вершины в соседях
            for neighbor in new_adjacency_list[vertex]:
                new_adjacency_list[neighbor[0]] = [
                    adj for adj in new_adjacency_list[neighbor[0]] if adj[0] != vertex
                ]
            # Удаляем саму висячую вершину
            del new_adjacency_list[vertex]

        # Создаем новый граф с обновленным списком смежности
        return Graph(self.directed, new_adjacency_list, self.weighted)
